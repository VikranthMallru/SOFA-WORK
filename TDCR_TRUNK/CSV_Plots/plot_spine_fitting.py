...
# ------- VISUAL: Consistent global limits with small margin -------
...
margin = 0  # Match new code margin
...
def get_axis_limits():
    # Now just return the above computed limits
    return x_lim, y_lim, z_lim

# Keep the rest as in original
...
slider.label.set_fontsize(16)  # Match new code
...
for text in check.labels:
    text.set_fontsize(16)  # Match new code
...
def plot_row(row_idx, mode_2d, show_spiral):
    global spine_scatter, tdcr_scatter, spiral_line, ax
    legend_ax.clear()
    legend_ax.axis('off')
    ...
    if mode_2d:
        ...
        ax.scatter(spine_2d[:, 0], spine_2d[:, 1], c='b', marker='o', s=40, label='Spine Points', visible=status[0])
        ax.scatter(tdcr_2d[:, 0], tdcr_2d[:, 1], c='g', marker='o', s=40, label='ROI Points', visible=status[1])
        if show_spiral:
            x = spine_2d[:, 0]
            y = spine_2d[:, 1]
            ...
            ax.plot(x_fit, y_fit, 'r-', lw=2, label='Fitted Spiral')
        ...
        # -------- VISUAL: Label font, tick font, legend consistent ---------
        f = 12
        lp = 20
        ...
        # ax.set_title('3D Spine and ROI Visualization', fontsize=18)
        ...
        # --------- VISUAL: Improved legend (external box) ---------
        legend_handles = []
        legend_labels = []
        if status[1]:  # ROI_Points visible
            legend_handles.append(tdcr_scatter)
            legend_labels.append('ROI Points')
        if status[0]:  # Spine visible
